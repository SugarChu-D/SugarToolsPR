...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let results = pollster::block_on(async {
        white2_snivy_search(ds_config, BW2Mode::Normal, workgroup_size).await
    });
...
fn build_text(results: &[W2SnivySearchResult]) -> String {
    ...
    for r in results {
        ...
        out.push_str("patrat_frames:\n");
        for (frame, poke) in &r.patrat_frames {
            let nature = poke.nature.as_ref().map(|n| n.name()).unwrap_or("None");
            out.push_str(&format!("{}:{}\n", frame, nature));
        }
